        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">ShortUrls</li>
                </ol>
            </div>
        </div>
    </div>
            ...
            <div class="col-12">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">ShortUrls List</h3>
                    </div>
                    <div class="card-body">
...
function getPermissions() {
    $('table#example1').DataTable({
        ...
        "bFilter": true,
        ajax: {
            url: "{{ route('admin.shortUrls.shortUrlsListAjax') }}",
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}'
            }
...
$(document).on('switchChange.bootstrapSwitch', '.permissionStatusUpdate', function(event, state) {
    ...
    $.ajax({
        type: 'POST',
        url: "{{ route('admin.shortUrls.changeStatusShortUrls') }}",
        data: {
            _token: "{{ csrf_token() }}",
            id: id,
            status: status
...
$(document).on('click', '.deletePermission', function() {
    ...
    $.ajax({
        type: 'POST',
        url: "{{ route('admin.shortUrls.deleteShortUrl') }}",
        data: {
            _token: "{{ csrf_token() }}",
            id: id
        },
